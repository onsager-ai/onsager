...
//! [`onsager_engine::nodes::Scheduler`] → `dispatch` → the *production*
//! [`AgentExecutor`] registered by [`default_executor_registry`] →
//! [`onsager_engine::scheduler::ClaudeCliRunner`] spawning a sandboxed fake
...
use onsager_engine::scheduler::{PlanRunner, service::default_executor_registry};
...
async fn scheduler_path_surfaces_repos_to_agent_spawn() {
    ...
    // `onsager_engine::scheduler::repo_env`.)
    let _guard = ENV_LOCK.lock().await;
    let cap = tempfile::tempdir().unwrap();
    let cap_file = cap.path().join("argv.txt");
    let (_dir, script) = fake_claude(&cap_file);
