...
//!   doesn't know. Surfaces from [`crate::nodes::dispatch`]; an individual
//!   `Executor::execute` impl will never construct this itself.
//! - **`Spine`** — a [`crate::nodes::SpineClient`] call failed. Most
//!   executors propagate this with `?`; the `#[from]` impl keeps the
//!   call sites quiet.
//! - **`ScriptFailed`** — the [`crate::nodes::script::ScriptExecutor`] ran a
...
use crate::nodes::SpineError;

/// Error returned by a runtime executor's `execute()` method (or by
/// [`crate::nodes::dispatch`] when the kind isn't registered).
#[derive(Debug, Error)]
