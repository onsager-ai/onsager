...
//! [`crate::nodes::dispatch`] / [`ExecutorRegistry`], which live in this
...
use crate::nodes::context::ExecutorContext;
use crate::nodes::dispatch::dispatch;
use crate::nodes::error::ExecutorError;
use crate::nodes::registry::ExecutorRegistry;
use crate::nodes::spine::SpineClient;
...
impl Scheduler {
    ...
    async fn persist_outputs(
        ...
        plan: &ExecutionPlan,
        node: &Node,
        outputs: crate::nodes::context::ExecutorOutputs,
...
mod tests {
    use super::*;
    use crate::nodes::spine::test_support::MockSpine;
    ...
    #[async_trait]
    impl crate::nodes::Executor for RecordingExecutor {
        ...
        async fn execute(
            &self,
            ctx: ExecutorContext,
        ) -> Result<crate::nodes::context::ExecutorOutputs, ExecutorError> {
            let mut seen = self.seen.lock().unwrap();
            for (id, _) in &ctx.inputs {
                seen.push(id.clone());
            }
            Ok(crate::nodes::context::ExecutorOutputs::empty())
        }
    }
    ...
    async fn entry_node_receives_declared_spec_inputs() {
        ...
        registry.register(Arc::clone(&recorder) as Arc<dyn crate::nodes::Executor>);
        let store = Arc::new(InMemoryPlanStore::new());
        let spine = Arc::new(MockSpine::default());
        let scheduler = Scheduler::new(
            Arc::new(registry),
    ...
    #[async_trait]
    impl crate::nodes::Executor for DeclaredEmptyExecutor {
        fn executor_kind(&self) -> &'static str {
            "noop"
        }
        fn declared_provenance(
        ...
        async fn execute(
            ...
            Ok(crate::nodes::context::ExecutorOutputs::declared_empty())
        }
    }
    ...
    #[async_trait]
    impl crate::nodes::Executor for FailingExecutor {
        fn executor_kind(&self) -> &'static str {
            "noop"
        }
        fn declared_provenance(
