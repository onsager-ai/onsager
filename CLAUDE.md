...
- **Postgres is the spine; events are the record, not the protocol** ([ADR 0029](docs/adr/0029-one-process-progressive-rebuild.md), amending ADR 0001). All durable truth — workflows, runs, sessions, artifacts, credentials — lives in Postgres, and every run writes an append-only audit trail to the `events` table that the dashboard reads back. Coordination inside the control plane is function calls; there is no event-RPC, no listener dispatch. Re-splitting along the Postgres seam is now being taken ([ADR 0030](docs/adr/0030-scaling-sessions-across-machines.md), amending this commitment): agent **sessions** may run on worker machines that dial in over a WebSocket and stream events back. The load-bearing half is kept — durable truth never leaves Postgres and **workers hold no database**; the control plane stays the only writer to the spine.
...
Useful env: `ONSAGER_BIND` (default `127.0.0.1:3002`), `ONSAGER_CLAUDE_BIN` (shim the agent CLI in tests), `ONSAGER_MCP_URL` (explicit MCP callback; else derived from `RAILWAY_PUBLIC_DOMAIN` on Railway so remote workers can reach it, else the bind address — ADR 0030), `ONSAGER_MAX_CONCURRENT_RUNS` (session concurrency cap, default 4 — ADR 0030), `ONSAGER_MACHINE_TOKEN` (fleet: shared secret machines present to dial in; unset → `/api/fleet/connect` fails closed — ADR 0030), `GITHUB_APP_ID/SLUG/PRIVATE_KEY`, `GITHUB_WEBHOOK_SECRET`, `GITHUB_CLIENT_ID/SECRET`, `ONSAGER_RESET_SCHEMA` (one-shot: drop+recreate the public schema at boot before migrating — #637; remove after use).

Run a worker (ADR 0030): `ONSAGER_CONTROL_URL=ws://<control>:3002 ONSAGER_MACHINE_TOKEN=<secret> ONSAGER_MACHINE_NAME=local onsager worker`. The worker needs the `claude` CLI and workspace clone deps, not Postgres. With no worker connected the control plane runs sessions in-process (unchanged).
