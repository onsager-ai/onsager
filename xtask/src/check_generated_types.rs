...
fn regenerate(root: &Path) -> Result<()> {
    ...
    // Portal and spine both host `#[ts(export)]` types whose
    // generated bindings land in `GENERATED_DIR` (spine joined the
    // cascade with spec #434 — `TriggerKind` and its variant tree).
    // Each `-p` runs its own libtest harness invocation, so we drive
    // them in sequence and require at least one matched test per crate.
