...
//! `DELETE` against `workflows` / `workflow_stages` post-Slice 4;
//! stiglab keeps a slim read-only copy for its in-process needs in
//! `routes/projects.rs` (replay-trigger).
//!
//! Lever D (#149): there is one table, owned by the spine, written
//! directly. The translation that used to live in
//! `workflow_spine_mirror.rs` (gate_kind → target_state + gates JSON,
//! repo + label → trigger_config JSON) is folded into this module,
//! where it is the single read/write boundary.
//!
//! Pool: spine `PgPool`. In production portal and spine point at the
//! same Postgres, so the spine pool exposed via `EventStore::pool()`
//! is what writes here. Workflow routes 503 when the spine pool is
//! unreachable — same posture as the rest of the workflow surface.
