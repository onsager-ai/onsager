...
fn build_registry() -> Vec<ToolDescriptor> {
    vec![
        ...
        ToolDescriptor {
            ...
            description: "Request write access (push + open PR) to a bound-but-unpinned repo in a workspace-scoped run. Binding a repo to the workspace is the write-consent boundary: for a bound repo this mints a least-privilege `contents:write` + `pull_requests:write` token scoped to that one repo and returns an authenticated `remote` to push to. For an unbound repo no token is minted. The pinned repo (named in the trigger) is pre-approved and uses the normal push path — do not call this for it.",
            category: ToolCategory::Destructive,
