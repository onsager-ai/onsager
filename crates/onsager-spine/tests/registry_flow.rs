...
async fn wipe(store: &EventStore, workspace: &str) {
    for sql in [
        ...
        if sql.contains('$') {
            ...
            let _ = sqlx::query(sql).execute(store.pool()).await;
        }
    }
}
...
async fn propose_then_approve_type_lifecycle() {
    ...
    // Three registry events should have landed on the spine for this workspace.
    let events: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM events \
         WHERE stream_type = 'registry' \
           AND data->>'type' IN ('type_proposed','type_approved','type_deprecated')",
    )
